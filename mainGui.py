...
class LibraryName(MayaQWidgetBaseMixin, QtWidgets.QDialog):
	...
	def Find_duplicate_names(self):
		...
		for index, NameKey1 in enumerate(key,
										 start=1):  # [{'Default'}, 'Postfixes', 'Base', 'Limbs', 'Fingers', 'Face', 'Other']
...
class MSL_RenameTool(MayaQWidgetBaseMixin, QtWidgets.QDialog):
	...
	itClickButtonUI  = QtCore.Signal(tuple)
	def __init__(self):
	...
	def menuUI(self):
		...
		# Selected UI
		self.Selected_menu_Act = QtWidgets.QAction(QtGui.QIcon(os.path.join(root_, "icons/layers.svg")), "Selected",
												   self)
	...
	def Set_ReName(self):
		...
		librname = {"listNameOld":[],"listNameNew":[],"listNameShapOld":[],"listNameShapNew":[]}
		...
		if Name:
			if Number:
				for start, i in enumerate(ListLongName, start=number_start):
					...
					#//// for signal SelectedUI ////
					typeOld = cmds.nodeType(i)
					shapeOld = False

					if typeOld == "transform":
						shapeOld = cmds.listRelatives(i, shapes=1, f=1)[0]

					#------------------------------------

					NewshortName = cmds.rename(i, NewshortName)

					ListLongName, NewlongName = self.retur_NewNamelongList(ListLongName, i, NewshortName)

					# //// for signal SelectedUI ////
					typeNew = cmds.nodeType(NewlongName)
					shapeNew = False

					if typeNew == "transform":
						shapeNew = cmds.listRelatives(NewlongName, shapes=1, f=1)[0]


					librname["listNameOld"].append(i)
					librname["listNameNew"].append(NewlongName)
					librname["listNameShapOld"].append(shapeOld)
					librname["listNameShapNew"].append(shapeNew)
			else:
				for index, i in enumerate(ListLongName):
					...
					ListLongName, NewlongName = self.retur_NewNamelongList(ListLongName, i, shortName)
		...
		self.itClickButtonUI.emit(librname)
	...
	def retur_NewNamelongList(self, ListLongName, longName, NewshortName):
		...
		for num, i in enumerate(
				ListLongName):  # ['|pSphere2', '|pSphere2|pCylinder2', '|pSphere2|pCylinder2|pSphere1',]
		...
		return ListLongName, NewLongName
			...
			if len(text) > len(old_text):  # items_add
				...
				if sectionText != Number or Number == "" and cursor_pos < start:  # items_removed in left_text! [left_text] >|< (sectionText != [Number]) [right_text]
					...
					if sectionText != Number:  # [Text]DEL====>[Number]<===DEL[Text] Delet Number!
						...
						if self.Start_index_Number <= 0 + len(
